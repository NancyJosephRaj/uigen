...
function getToolLabel(toolName: string, args: Record<string, unknown>): string {
  ...
  if (toolName === "str_replace_editor") {
    switch (args.command) {
      case "create":     return `Creating ${filename(args.path)}`;
      case "str_replace":
      case "insert":     return `Editing ${filename(args.path)}`;
      case "view":       return `Reading ${filename(args.path)}`;
      default:           return `Working on ${filename(args.path)}`;
    }
  }

  if (toolName === "file_manager") {
    switch (args.command) {
      case "rename": return `Renaming ${filename(args.path)} → ${filename(args.new_path)}`;
      case "delete": return `Deleting ${filename(args.path)}`;
